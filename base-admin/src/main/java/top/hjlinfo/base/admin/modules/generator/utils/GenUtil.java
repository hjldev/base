package top.hjlinfo.base.admin.modules.generator.utils;
...
public class GenUtil {
    ...
    public static void generatorCode(List<ColumnInfo> columnInfos, GenConfig genConfig, String tableName) throws IOException {
        ...
        for (ColumnInfo column : columnInfos) {
            ...
            String changeColumnName = StrUtil.toCamelCase(column.getColumnName().toString());
            ...
            if(!StrUtil.isBlank(column.getColumnQuery())){
        ...
        // 生成权限
        Template template = engine.getTemplate("generator/admin/permission_sql.ftl");
        ...
        String filePermissionPath = packagePath + File.separator + "permission.sql";

        File filePermission = new File(filePermissionPath);

        // 如果非覆盖生成
        if(!genConfig.getCover()){
            if(!FileUtil.exist(filePermission)){
                genFile(filePermission, template, map);
            }
        } else {
            genFile(filePermission, template, map);
        }
